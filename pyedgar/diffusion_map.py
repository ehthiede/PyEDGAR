...
class DiffusionAtlas(object):
    ...
    def __init__(self,nneighbors=600,rho=None,point_weights=None,d=None,alpha='0',beta='-1/(d+2)',epses=2.**np.arange(-40,41),rho_norm=True,metric='euclidean',metric_params=None):
        ...
        d : int or None, optional
            Dimension of the system. If None and alpha or beta settings require the dimensionality, the dimension is estimated using the kernel density estimate, if a kernel density estimate is performed.
        alpha : float or string, optional
            Parameter for left normalization of the Diffusion map.  Either a float, or a string that can be interpreted as a mathematical expression.  The variable "d" stands for dimension, so "1/d" sets the alpha to one over the system dimension.  Default is 0
