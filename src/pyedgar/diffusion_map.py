...
class DiffusionAtlas(object):
    ...
    def fit(self, data, rho=None, point_weights=None):
        ...
        if isinstance(self.epses,numbers.Number):
            ...
            if self.verbosity >= 1 : print("Epsilon automatically detected to be : %f"%epsilon)
            if d is None: # If dimensionality is not provided, use estimated value.
                d = d_est
                if self.verbosity >= 1 : print("Dimensionality estimated to be %d."% d)
        ...
        if self.verbosity >= 2 : print("Evaluated Kernel")
        ...
        if alpha != 0:
            ...
            K = diagq * K 
            K = K * diagq
            if self.verbosity >= 2 : print(r"Applied q**\alpha normalization.")
        ...
        if self.rho_norm:
            ...
            pi = q_alpha
        L = diag_norm * L
        if self.verbosity >= 2 : print("Normalized matrix to transition rate matrix.")
        ...
        if self.rho_norm:
            ...
            pi = q_alpha
        pi /= np.sum(pi)
        if self.verbosity >= 2 : print("Estimated Stationary Distribution.")
